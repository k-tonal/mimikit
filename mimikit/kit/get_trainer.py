...
def get_trainer(root_dir: str = None,
    ...
    kwargs
        ...
        See `the Trainer documentation <https://pytorch-lightning.readthedocs.io/en/latest/trainer.html#trainer-class-api>`__ for full references.
    ...
    ------
    TypeError
        if ``version`` is neither ``None`` nor an ``int``.

    ValueError
        if ``neptune_connector`` is not ``None`` while ``model`` is.

    Notes
    -----
    .. note::
        Differences to the default ``Trainer`` in lightning are :
